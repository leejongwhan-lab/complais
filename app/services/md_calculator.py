...
class MDCalculatorService:
    """CB Admin Dashboard용 파사드 — plus_pct/minus_pct 별칭을 기존 엔진에 연결."""
    ...
    def calculate_review_md(
        ...
        is_integrated: bool = False,
        *,
        add_pct: Optional[int] = None,
        subtract_pct: Optional[int] = None,
    ) -> Tuple[float, float, float]:
        return calculate_review_md(
            base_md=base_md,
            add_pct=add_pct if add_pct is not None else plus_pct,
            subtract_pct=subtract_pct if subtract_pct is not None else minus_pct,
