...
class IsoStandardOption(BaseModel):
    ...
    clauses_status: Optional[str] = None
...
def _list_available_standards(db: Session) -> List[IsoStandardOption]:
    ...
    rows = (
        ...
        .filter(
            StandardMaster.is_active.is_(True),
            StandardMaster.role == "CERTIFIABLE",
        )
        .order_by(StandardMaster.family_code.asc(), StandardMaster.edition_year.asc())
        .all()
    )
    if rows:
        return [
            IsoStandardOption(
                ...
                family_code=getattr(r, "family_code", None),
                edition_year=getattr(r, "edition_year", None),
                clauses_status=getattr(r, "clauses_status", None),
            )
            for r in rows
        ]
