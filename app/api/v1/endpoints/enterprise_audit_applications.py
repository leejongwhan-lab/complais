...
from app.core.security import CurrentUser, get_current_user, require_cb_review_access
...
def cb_review_application(
    ...
    db: Session = Depends(get_db),
    current_user: CurrentUser = Depends(require_cb_review_access),
):
    """③ MD 제안검토(가감/입회/상태) — platform_admin 또는 향후 CB 역할.

    final = base × (1 + ratio/100). CB 포털 UI는 아직 없으며 관리자 대시보드에서 사용.
    """
    row = db.get(EnterpriseAuditApplication, application_id)
    if not row:
        raise HTTPException(status_code=404, detail="신청건을 찾을 수 없습니다.")
    ...
    role = (getattr(current_user, "role", None) or "").lower()
    is_platform_admin = role == "platform_admin" or "platform" in role
    if user_cb and int(user_cb) != int(row.cb_id) and not is_platform_admin:
